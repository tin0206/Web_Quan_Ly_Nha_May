    ...
    public class ProductionOrdersController : ControllerBase
    {
        ...
        )
        {
            ...
            if (!string.IsNullOrWhiteSpace(statuses))
            {
                ...
                if (arr.Contains("Bình thường"))
                    conds.Add("po.Status = 1");

                if (arr.Contains("Đã hủy"))
                    conds.Add("po.Status = -1");
        ...
        )
        {
            ...
            if (!string.IsNullOrWhiteSpace(processAreas))
            {
                ...
                if (arr.Any())
                {
                    ...
                    where.Add($"po.ProcessArea IN ({string.Join(",", ps)})");
                }
            }
            ...
            if (!string.IsNullOrWhiteSpace(shifts))
            {
                ...
                if (arr.Any())
                {
                    ...
                    where.Add($"po.Shift IN ({string.Join(",", ps)})");
                }
            }
                ...
                if (arr.Any())
                {
                    ...
                    where.Add($@"
                        EXISTS (
                            SELECT 1
                            FROM Batches b
                            WHERE b.ProductionOrderId = po.ProductionOrderId
                            AND b.BatchId IN ({string.Join(",", ps)})
                        )");
                }
            }
            ...
            var batchesByPo = batches
                .GroupBy(b => (int)b.ProductionOrderId)
            ...
            var data = rows.Select(o => new
            {
                ...
                batches = batchesByPo.ContainsKey((int)o.ProductionOrderId)
                    ? batchesByPo[(int)o.ProductionOrderId]
                    : new List<object>()
            });
